...
\begin{titlepage}
    ...
    % Logo DEI
    \includegraphics[width=0.6\textwidth]{DEI.png}
...
\node (barrel2) [barrel, below of=gateway, yshift=-0.4cm] {Barrel 2\\Port 8184};
\node (barrel3) [barrel, below right of=gateway, xshift=3cm, yshift=-1cm] {Barrel 3\\Port 8185};

% Queue
\node (queue) [component, right of=gateway, xshift=3.5cm] {URL Queue\\Port 8181};
 
% Downloader
\node (downloader) [service, below of=gateway, yshift=-2.5cm] {Downloader(s)};

% Arrows
\draw [arrow] (frontend) -- node[anchor=west] {HTTPS/JSON} (webapp);
\draw [arrow] (webapp) -- node[anchor=west] {RMI} (gateway);
\draw [arrow] (gateway) -- (webapp);
...
% Route from right side of URL Queue, over components
\draw [arrow] (queue.east) to[out=0, in=270, looseness=1.2] (downloader.south);
% Downloader indexes into barrels
\draw [arrow, bend left=60] (downloader) to (barrel1);
...
\begin{tikzpicture}
% Left column
\node (top10) [component, minimum width=5.5cm, minimum height=3.25cm] at (0,1.875) {Top 10 Searches};
\node (tempo) [component, minimum width=5.5cm, minimum height=3.25cm] at (0,-1.875) {Avg Response Time};
% Right column (with more spacing)
\node (barrels) [component, minimum width=7cm, minimum height=7cm] at (6.75,0) {Barrels Status};
\end{tikzpicture}
...
\begin{longtable}{|p{1cm}|p{5cm}|p{5.5cm}|p{2.2cm}|}
\hline
